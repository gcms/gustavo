...
class DecolarProviderTests extends GroovyTestCase {
    void setUp() {
        ...
        new File('testdb').eachFile { File file ->
            String key = 'decolar' + today + file.name
            Map map = [key: key, content: new JsonSlurper().parseText(file.text).conteudo]
            Cache.DEFAULT.put(key, map)
        }
    }
    ...
    void testParse() {
//        String key = DespegarLoader.getKey(new URL("http://www.us.despegar.com/shop/flights/data/search/roundtrip/gyn/dub/2014-02-07/2014-03-05/1/0/0/TOTALFARE/ASCENDING/NA/NA/NA/NA/NA"))
//
//        Cache.DEFAULT.remove(key)
//        Cache.DEFAULT.put(key, getClass().getResourceAsStream('/roundTripGYNDUB.json').newReader().text)

        Provider provider = new DespegarProvider()
...
//        Cache.DEFAULT.put(key, getClass().getResourceAsStream('/roundTripGYNDUB.json').newReader().text)
    ...
    void testOneWay() {
//        String key = DespegarLoader.getKey(new URL("http://www.us.despegar.com/shop/flights/data/search/oneway/bsb/mia/2014-02-08/1/0/0/TOTALFARE/ASCENDING/NA/NA/NA/NA"))
//
//        Cache.DEFAULT.remove(key)
