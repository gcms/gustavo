...
#define CONTING_LOAD_GET_PRIVATE(o) (G_TYPE_INSTANCE_GET_PRIVATE((o), \
        CONTING_TYPE_LOAD, ContingLoadPrivate))
...
typedef struct ContingLoadPrivate_ ContingLoadPrivate;
struct ContingLoadPrivate_ {
    gboolean dragging;
...
static void
conting_load_finalize(GObject *self)
{
    ContingLoadPrivate *priv;
...
static void conting_load_delete(ContingDrawing *self)
{
    ContingLoadPrivate *priv;
...
static void
conting_load_link_deleted(ContingComponent *comp,
                         ContingDrawing *drawing)
{
    ContingLoadPrivate *priv;

    g_return_if_fail(comp != NULL && CONTING_IS_LOAD(comp));

    priv = CONTING_LOAD_GET_PRIVATE(comp);
...
static gboolean
conting_load_link(ContingComponent *self,
                    ContingDrawing *drawing,
                    ...
                    ArtPoint *pw)
{
    ...
    g_return_val_if_fail(self != NULL && CONTING_IS_LOAD(self), FALSE);
...
static xmlNodePtr
conting_load_xml_node(ContingDrawing *self,
		                xmlNodePtr drawing_node)
{
	...
	g_return_val_if_fail(self != NULL && CONTING_IS_LOAD(self), NULL);
	...
	xmlNewProp(class_node, BAD_CAST "name",
			BAD_CAST g_type_name(CONTING_TYPE_LOAD));
...
static void
conting_load_place_xml(ContingDrawing *self, xmlNodePtr drawing_node,
                         GHashTable *id_drawing)
{
    ...
    g_print("conting_load_place_xml()\n");
    ...
    for (class_node = drawing_node->children; class_node;
        ...
        if (class_name && xmlStrEqual(class_name, "ContingLoad")) {
            xmlNodePtr attr;
...
static void
conting_load_get_bus(ContingDrawing *self, ContingDrawing *linked,
		ContingComponent **comp)
{
	ContingLoadPrivate *priv;
...
conting_load_class_init(gpointer g_class, gpointer class_data)
{
    ...
    drawing_class->delete = conting_load_delete;
	drawing_class->xml_node = conting_load_xml_node;
	drawing_class->place_xml = conting_load_place_xml;

	drawing_class->get_bus = conting_load_get_bus;

    component_class = CONTING_COMPONENT_CLASS(g_class);
    component_class->link = conting_load_link;
    component_class->link_deleted = conting_load_link_deleted;

    gobject_class = G_OBJECT_CLASS(g_class);
    gobject_class->finalize = conting_load_finalize;

    g_type_class_add_private(g_class, sizeof(ContingLoadPrivate));
